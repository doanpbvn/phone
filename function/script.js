            ...
            const bookmarkletCode = `(async function() {
                ...
                async function performFakeClean(deviceIDs) {
                    for (const deviceID of deviceIDs) {
                        await fetch(\`https://ifake.pro/manager/device/\${deviceID}/tools\`, {headers: {accept: "*/*","content-type": "application/x-www-form-urlencoded; charset=UTF-8"},
                            referrer: \`https://ifake.pro/manager/device/\${deviceID}\`,referrerPolicy: "strict-origin-when-cross-origin",body: "action=fake_clean",method: "POST",mode: "cors",credentials: "include"});}
                    setTimeout(() => {console.log("%c✅ Fake Xong. Check trên iPhone nhé!", "color: green; font-weight: bold;");console.log("%c!--------------------------------!", "color: gray; font-weight: bold;");}, 30000);}})();`;
